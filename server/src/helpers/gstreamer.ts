...
export function buildGStreamerPipeline(consumersInfo: ConsumerInfo[], outputFile: string): string {
  ...
  const basePipeline =
    `compositor name=comp ! videoconvert ! x264enc tune=zerolatency ! mp4mux name=mux streamable=true ! filesink location=${outputFile} ` +
...
export function startGStreamer(consumersInfo: ConsumerInfo[], outputFile: string): ChildProcessByStdio<null, null, Readable> {
  ...
  const process = spawn("gst-launch-1.0", args, { stdio: ["inherit", "inherit", "pipe"] });
