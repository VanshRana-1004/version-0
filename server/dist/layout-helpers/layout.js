...
async function createLayout(segment, outputPath) {
    ...
    await new Promise((resolve, reject) => {
        const ff = (0, child_process_1.exec)(command, (err, stdout, stderr) => {
            if (err)
                return reject(err);
            resolve();
        });
